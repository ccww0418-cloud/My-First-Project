...
if [ -n "$APIGW_ID" ] && [ "$APIGW_ID" != "None" ]; then
  ...
  if [ "${MAX_TOOL_ITERATIONS:-4}" -gt 3 ]; then
    MAX_TOOL_ITERATIONS=3
    info "MAX_TOOL_ITERATIONS=3 (API Gateway 통합 타임아웃 30초 대응)"
  fi
...
OPENAI_BUDGET_MS=${OPENAI_BUDGET_MS:-10000},\
OPENAI_ANSWER_RESERVE_MS=${OPENAI_ANSWER_RESERVE_MS:-5000},\
